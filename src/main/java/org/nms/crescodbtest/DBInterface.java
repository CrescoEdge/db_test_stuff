...
import testhelpers.PluginBuilder;
import testhelpers.CLogger;
...
public class DBInterface {
    ...
    //NMS added this constructor so I could pass an in-memory db to a controllerEngine
    public DBInterface(ControllerEngine controllerEngine, ODatabaseDocumentTx db) {
        this.controllerEngine = controllerEngine;
        this.plugin = controllerEngine.getPluginBuilder();
        this.logger = plugin.getLogger(DBInterface.class.getName(),CLogger.Level.Info);
        ...
        //this.agentcontroller = agentcontroller;
        this.importQueue = new LinkedBlockingQueue<>();
        this.gde = new DBEngine(controllerEngine,db);
        this.gdb = new DBBaseFunctions(controllerEngine,gde);
        this.dba = new DBApplicationFunctions(controllerEngine,gde);
        this.gson = new Gson();
        this.type = new TypeToken<Map<String, List<Map<String, String>>>>() {
    ...
    //NMS method 'getResourceTotal2()' doesn't seem to be in use anymore so I deleted it
